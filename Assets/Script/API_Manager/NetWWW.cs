...
public class WebLogin : IWebData
{
    ...
    public string URL() { return "Login2.php"; }
    ...
    public void Recv(JsonData JsonObject)
    {
        ...
        //NetWWW.INSTANCE().MessageBox(NetWWW.INSTANCE().session);
...
{
    const string PHP_URL = "http://14.52.96.94:4600/apitest/";

    // 객체들을 각각의 패킷으로 취급, 직렬화하여 순차적으로 전송 (디자인 패턴 중 Command Pattern 참고)
    /*static*/
    List<IWebData> _Sendlist = new List<IWebData>();
